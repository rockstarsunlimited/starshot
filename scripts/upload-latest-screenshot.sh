...
SCRIPT_DIR="${0:A:h}"
source "$SCRIPT_DIR/starshot-lib.sh"
...
case "${STARSHOT_UPLOAD_FORMAT:l}" in
  jpeg|jpg)
    STARSHOT_JPEG_QUALITY="${STARSHOT_JPEG_QUALITY//[^0-9]/}"
    ...
    UPLOAD_FILE="$STARSHOT_WORK_DIR/${LATEST_FILE:t:r}.jpg"
    ...
    ;;
  png)
    UPLOAD_FILE="$STARSHOT_WORK_DIR/${LATEST_FILE:t:r}.png"
...
if [[ -n "$STARSHOT_CLEANUP_DAYS" ]]; then
  STARSHOT_CLEANUP_DAYS="${STARSHOT_CLEANUP_DAYS//[^0-9]/}"
  if [[ -n "$STARSHOT_CLEANUP_DAYS" ]]; then
